...
public class Jwork {
        ...
        public static void main(String[] args) {
            ...
            DatabaseRecruiter.addRecruiter(new Recruiter(DatabaseRecruiter.getLastId()+1, "bryandario", "bryandario@gmail.com", "08218312736", location3));

            try{
                DatabaseJob.addJob(new Job(DatabaseJob.getLastId()+1, "Bryan1", DatabaseRecruiter.getRecruiterById(1), 80000, JobCategory.WebDeveloper));
            }catch (RecruiterNotFoundException error){
                System.out.println(error.getMessage());
            }
            try{
                DatabaseJob.addJob(new Job(DatabaseJob.getLastId()+1, "Bryan2", DatabaseRecruiter.getRecruiterById(2), 80000, JobCategory.WebDeveloper));
            }catch (RecruiterNotFoundException error){
                System.out.println(error.getMessage());
            }
            try{
                DatabaseJob.addJob(new Job(DatabaseJob.getLastId()+1, "Bryan3", DatabaseRecruiter.getRecruiterById(3), 80000, JobCategory.BackEnd));
            }catch (RecruiterNotFoundException error){
                System.out.println(error.getMessage());
            }
            try{
                DatabaseJob.addJob(new Job(DatabaseJob.getLastId()+1, "Bryan4", DatabaseRecruiter.getRecruiterById(3), 80000, JobCategory.FrontEnd));
            }catch (RecruiterNotFoundException error){
                System.out.println(error.getMessage());
            }
